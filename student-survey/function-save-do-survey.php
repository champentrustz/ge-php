...
for($j = 0 ; $j<$numrow ; $j++){
    ...
    $survey_score_sql="SELECT * FROM survey_score WHERE survey_course_id = '".$survey_course_id."' and student_id = '".$student_id."' and survey_question_id = '".$question_id_i."'";
    $query_survey_score=mysqli_query($conn,$survey_score_sql);
    $count_row_score = $query_survey_score->num_rows;

    if($count_row_score == 0) {
...
if($remark != '') {
    ...
    $survey_remark_sql="SELECT * FROM survey_remark WHERE survey_course_id = '".$survey_course_id."' and student_id = '".$student_id."'";
    $query_survey_remark=mysqli_query($conn,$survey_remark_sql);
    $count_row_remark = $query_survey_remark->num_rows;

    if($count_row_remark == 0) {
